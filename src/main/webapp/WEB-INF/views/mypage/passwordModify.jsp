...
<body>
	<form name="passwordModifyForm" action="passwordModify" method="post">
		<table>
			<tr><td>아이디</td><td><input type=text name=userid id=userid value="${member.userid }" readonly></td></tr>
			<tr><td>현재 비밀번호</td><td><input type=password name=pw id=pw required></td></tr>
			<tr><td>신규 비밀번호</td><td><input type=password name=changePw id=changePw required oninput=changePwCheck()><div id="changePw_check"></div></td></tr>
			<tr><td>비밀번호 확인</td><td><input type=password name=changePw2 id=changePw2 required oninput=changePw2Check()><div id=changePw2_check></div></td></tr>
		</table>
		<div>
			<button id=btn name=btn onclick='check()' disabled>비밀번호 변경</button>
